...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "bibliotech-ear.ear");

        // Current directory is the root of the "bibliotech-web" project. Go up one level
        // and enter the "bibliotech-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../bibliotech-ejb/target/bibliotech-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "bibliotech-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/bibliotech-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "bibliotech-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("com.bibliotech.test");
